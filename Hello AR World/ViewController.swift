...
//  Hello AR World
//
//  Created by Levi Davis on 1/17/20.
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    private func drawSphereAtOrigin() {
        self.sphere = SCNNode(geometry: SCNSphere(radius: 0.05))
    ...
    private func drawBoxAt1200High() {
        ...
        box.position = SCNVector3(0.0, 0.2, -0.3)
    ...
    private func drawPyramidAt600Low() {
        let pyramid = SCNNode(geometry: SCNPyramid(width: 0.1, height: 0.1, length: 0.1))
        pyramid.position = SCNVector3(0.0, -0.2, 0.3)
    ...
    private func drawPlaneAt900() {
        let plane = SCNNode(geometry: SCNPlane(width: 0.1, height: 0.1))
        plane.position = SCNVector3(-0.2, 0.0, 0.0)
    ...
    private func drawTorasAt300() {
        let toras = SCNNode(geometry: SCNTorus(ringRadius: 0.05, pipeRadius: 0.03))
        toras.geometry?.firstMaterial?.diffuse.contents = UIColor.red
        toras.geometry?.firstMaterial?.specular.contents = UIColor.white
        toras.position = SCNVector3(0.2, 0.0, 0.0)
        toras.eulerAngles = SCNVector3(0, 0, 45.degreesToRadians())
        sceneView.scene.rootNode.addChildNode(toras)
    }
